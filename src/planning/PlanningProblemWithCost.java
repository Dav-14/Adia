...
public class PlanningProblemWithCost extends PlanningProblem {
    ...
    public PlanningProblemWithCost(State ini, State goal, List<Action> act) {
        super(ini, goal, act);
        this.cost = new HashMap<Action, Integer>();
        for (Action action : act) {
            int entier;
            if (action.equals(HealthCare.SYRUP_BUTTON_HIGH) || action.equals(HealthCare.SYRUP_BUTTON_MEDIUM)
                    || action.equals(HealthCare.SYRUP_BUTTON_LOW)) {
                entier = 2;
            } else {
                entier = 1;
            }
            cost.put(action, entier);
        }
    ...
    /**
     * Trouve le State ayant la plus petite distance parmis les States du paramètres
     * open
     * 
    ...
    /**
     * Trouve un Stack (une pile) d'action permettant de passer de l'état initial à
     * l'état final d'un problème en utilisant l'algorithme de Dijkstra
     * 
     ...
     */
    protected static Stack<Action> reverseStackDijkstra(Map<State, State> father, Map<State, Action> actions,
